<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>assets/easyui/themes/default/easyui.css">
    ...
    <div id="dlg" class="easyui-dialog" style="width:400px" data-options="closed:false,modal:true,border:'thin',buttons:'#dlg-buttons'" title="Laporan Jenis">
        <form id="fm" method="post" novalidate style="margin:0;padding:20px 50px">
            <div style="margin-bottom:10px">
                <p><input name="filter"
                id="kode" type="radio" checked="checked">Berdasarkan Kode Jenis: </p>
                <input name="kd_jenis" id="kd_jenis" class="easyui-textbox" style="width:100%">
            </div>
            <div style="margin-bottom:10px">
                <p><input name="filter"
                id="nama" type="radio">Berdasarkan Nama Jenis: </p>
                <input name="nm_jenis" id="nm_jenis" class="easyui-textbox" style="width:100%">
            </div>
        </form>
    </div>
    <div id="dlg-buttons">
    ...
    <script type="text/javascript">
        function Laporan(ket){
            if(document.getElementById('kode').checked==true){
                var datalaporan="filter=kode&kd_jenis="+document.getElementById('kd_jenis').value;
            }
            if(document.getElementById('nama').checked==true){
                var datalaporan="filter=nama&nm_jenis="+document.getElementById('nm_jenis').value;
            }
            if(ket=='excel'){
                window.open('<?php echo base_url(); ?>Lapjenis/cetakexcel?'+datalaporan);
            }else{
                window.open('<?php echo base_url(); ?>Lapjenis/cetakpdf?'+datalaporan);
            }
        }
    </script>