<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>assets/easyui/themes/default/easyui.css">
    ...
    <div id="dlg" class="easyui-dialog" style="width:400px" data-options="closed:false,modal:true,border:'thin',buttons:'#dlg-buttons'" title="Laporan Pembelian">
        <form id="fm" method="post" novalidate style="margin:0;padding:20px 50px">
            <div style="margin-bottom:10px">
                <p><input name="filter"
                id="kode" type="radio" checked="checked">Berdasarkan Kode  Pembelian: </p>
                <input name="kd_pembelian" id="kd_pembelian" class="easyui-textbox" style="width:100%">
            </div>
            <div style="margin-bottom:10px">
                <p><input name="filter"
                id="tanggal" type="radio">Berdasarkan Tanggal: </p>
                <input name="tgl_pembelian" id="tgl_pembelian" class="easyui-textbox" style="width:100%">
            </div>
        </form>
    </div>
    <div id="dlg-buttons">
    ...
    <script type="text/javascript">
        function Laporan(ket){
            if(document.getElementById('kode').checked==true){
                var datalaporan="filter=kode&kd_pembelian="+document.getElementById('kd_pembelian').value;
            }
            if(document.getElementById('tanggal').checked==true){
                var datalaporan="filter=tanggal&tgl_pembelian="+document.getElementById('tgl_pembelian').value;
            }
            if(ket=='excel'){
                window.open('<?php echo base_url(); ?>Lappembelian/cetakexcel?'+datalaporan);
            }else{
                window.open('<?php echo base_url(); ?>Lappembelian/cetakpdf?'+datalaporan);
            }
        }
    </script>